...
class ArtifactLibraryService:
    ...
    def create_text_artifact(self, kind: str, name: str, content: str, trace_id: str | None = None) -> ArtifactRecord:
        ...
        record = ArtifactRecord(kind=kind, name=name, path=path, trace_id=trace_id)
        self._records.append(record)
        self.context.registry.emit(
            "artifact.created",
            {"kind": record.kind, "name": record.name, "path": str(record.path), "trace_id": trace_id},
        )
        return record

    def register_external_artifact(self, kind: str, name: str, path: Path, trace_id: str | None = None) -> ArtifactRecord:
